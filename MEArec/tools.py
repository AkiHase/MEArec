            ...
            if bcat == 'E':
                ...
                if n_sel_inh < n_inh:
                    ...
                    if np.any(dist < min_dist):
                        ...
                        amp = np.max(np.abs(np.min(templates[id_cell])))
                        if not drifting:
                            if is_position_within_boundaries(loc[id_cell], x_lim, y_lim, z_lim) and \
                                    min_amp < amp < max_amp:
                                # save cell
                                if n_overlap_pairs is None:
                                    pos_sel.append(loc[id_cell])
...
def annotate_overlapping_spikes(spiketrains, t_jitt=1*pq.ms, overlapping_pairs=None, n_jobs=None,
    ...
    n_jobs : int
        Number of jobs for parallel processing. If None half cpus are used
    parallel : bool
        If True spike trains are processed in parallel with joblib
    verbose : bool
        If True output is verbose
    ...
    if parallel:
        ...
        output_list = Parallel(n_jobs=n_jobs)(delayed(_annotate_parallel)(i, st_i, spiketrains, t_jitt,
                                                                          overlapping_pairs, verbose,)
                                                      for i, st_i in enumerate(spiketrains))
        for i, st in enumerate(spiketrains):
            spiketrains[i] = output_list[i]
    else:
...
def plot_rasters(spiketrains, cell_type=False, ax=None, overlap=False, color=None, fs=10,
    ...
    spiketrains : list
        List of neo spike trains
    cell_type : bool
        If True and 'cell_type' in spike train annotation spike trains are plotted based on their type
    ax : axes
